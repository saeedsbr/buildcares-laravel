...
<section class="section-padding relative overflow-hidden" style="background-color:#ffffff;">
        ...
        <svg class="w-full h-full" viewBox="0 0 1440 600" preserveAspectRatio="xMidYMid slice">
            <defs>
                <pattern id="fp-grid" width="40" height="40" patternUnits="userSpaceOnUse">
                    <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#2563eb" stroke-width="0.6"/>
                </pattern>
            </defs>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
        <div class="flex flex-wrap items-end justify-between gap-6 mb-12">
            <div>
                ...
                <h2 class="section-title reveal">Project Highlights</h2>
                <p class="leading-relaxed max-w-xl reveal" style="color:#64748b;">
                    A closer look at three of our most-requested project types — loft conversions, double-storey side extensions and single-storey rear extensions.
                </p>
            </div>
            <a href="{{ route('portfolio.index') }}" class="btn-outline-gold reveal text-xs">
                View Full Portfolio
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($featuredProjects as $i => $fp)
            @php $p = $fp['item']; @endphp
            <a href="{{ route('portfolio.show', $p->slug) }}"
               class="group block reveal" style="animation-delay:{{ $i * 0.1 }}s;">
                {{-- Drawing-sheet card --}}
                <div class="relative border transition-all duration-300 group-hover:shadow-xl group-hover:-translate-y-1"
                     style="background:#ffffff; border-color:#dbeafe;">

                    {{-- Image area with drawing-corner crops --}}
                    <div class="relative aspect-[4/3] overflow-hidden" style="background:#f0f7ff;">
                        <img src="{{ Storage::url($p->cover_image) }}" alt="{{ $p->title }}" loading="lazy"
                             class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">

                        {{-- Subtle blueprint grid overlay (sits faintly above the image) --}}
                        <svg class="absolute inset-0 w-full h-full opacity-0 group-hover:opacity-[0.18] transition-opacity duration-300 pointer-events-none" viewBox="0 0 400 300" preserveAspectRatio="none">
                            <defs>
                                <pattern id="fp-card-grid-{{ $i }}" width="20" height="20" patternUnits="userSpaceOnUse">
                                    <path d="M 20 0 L 0 0 0 20" fill="none" stroke="#2563eb" stroke-width="0.6"/>
                                </pattern>
                            </defs>
                            <rect width="400" height="300" fill="url(#fp-card-grid-{{ $i }})"/>
                        </svg>

                        {{-- Corner crops --}}
                        <div class="absolute top-2 left-2 w-4 h-4 border-l border-t pointer-events-none" style="border-color:rgba(37,99,235,0.55);"></div>
                        <div class="absolute top-2 right-2 w-4 h-4 border-r border-t pointer-events-none" style="border-color:rgba(37,99,235,0.55);"></div>
                        <div class="absolute bottom-2 left-2 w-4 h-4 border-l border-b pointer-events-none" style="border-color:rgba(37,99,235,0.55);"></div>
                        <div class="absolute bottom-2 right-2 w-4 h-4 border-r border-b pointer-events-none" style="border-color:rgba(37,99,235,0.55);"></div>

                        {{-- Category badge --}}
                        <div class="absolute top-3 left-3 text-[10px] uppercase tracking-[0.18em] font-bold px-2 py-1" style="background:rgba(37,99,235,0.95); color:#ffffff;">
                            {{ $fp['label'] }}
                        </div>

                        {{-- Sheet number --}}
                        <div class="absolute top-3 right-3 text-right pointer-events-none">
                            <div class="text-[9px] font-bold" style="color:rgba(37,99,235,0.7);">BuildCares</div>
                            <div class="text-[8px]" style="color:rgba(37,99,235,0.5);">{{ sprintf('Sheet %02d / %02d', $i + 1, $featuredProjects->count()) }}</div>
                        </div>
                    </div>

                    {{-- Title strip (like a drawing title block) --}}
                    <div class="border-t px-5 py-4" style="border-color:#dbeafe;">
                        <div class="flex items-center gap-2 mb-1.5 text-[10px] uppercase tracking-[0.15em]" style="color:#94a3b8;">
                        ...
                        <h3 class="font-bold text-lg mb-3 leading-snug transition-colors group-hover:text-blue-600" style="color:#0f172a;">
                            {{ $p->title }}
                        </h3>
                        @if($p->description)
                        <p class="text-sm leading-relaxed line-clamp-2 mb-4" style="color:#64748b;">
                            {{ $p->description }}
                        </p>
                        @endif

                        {{-- Footer (matches a CAD title-block layout) --}}
                        <div class="flex items-center justify-between pt-3 border-t" style="border-color:#e2e8f0;">
                            <span class="text-[10px] uppercase tracking-[0.15em]" style="color:#94a3b8;">Scale 1:50 · Rev A</span>
                            <span class="text-xs font-bold uppercase tracking-widest flex items-center gap-1.5 group-hover:gap-2.5 transition-all" style="color:#2563eb;">
                                View
